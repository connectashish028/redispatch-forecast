...
Writes:
  ...
  data/predictions/contributions_<YYYY-MM-DD>.parquet   (TreeSHAP, 24h horizon)
  data/predictions/contributions_latest.parquet   (copy of the most recent run)

  Contribution outputs are emitted by default; pass --skip-contribs to disable.
...
def score(target_date: pd.Timestamp,
          want_contribs: bool = True
          ) -> tuple[pd.DataFrame, pd.DataFrame | None]:
    """Run the three models over all towns × all hours for target_date.

    Returns
    -------
    predictions  DataFrame with calibrated + raw probabilities, geo, metadata.
    contribs     If want_contribs: DataFrame with per-feature TreeSHAP
                 contributions for the 24h model (one row per (ts, town),
                 columns = feature_cols + 'bias'). All in log-odds space.
                 Else: None.
    """
    ...
    # 5b. TreeSHAP contributions for the 24h model only — that's the horizon
    # the dashboard's "why?" panel decomposes. Skipping 1h/6h keeps the
    # artifact small (~24 × 175 × 199 floats per day ≈ 3 MB).
    contribs = None
    if want_contribs:
        # pred_contrib=True returns shape (n_rows, n_features + 1); last col
        # is the bias / intercept. All values in log-odds space.
        shap = models['y_24h'].predict(X, pred_contrib=True)
        contrib_cols = list(fcols) + ['bias']
        contribs = pd.DataFrame(shap, columns=contrib_cols)
        contribs.insert(0, 'town', raw_town.values)
        contribs.insert(0, 'ts',   sub['ts'].values)
    ...
    out_sorted = out[cols].sort_values(['ts', 'town']).reset_index(drop=True)

    if contribs is not None:
        contribs = contribs.sort_values(['ts', 'town']).reset_index(drop=True)

    return out_sorted, contribs
...
def main():
    ...
    ap.add_argument('--skip-contribs', action='store_true',
                    help="Don't compute or write the TreeSHAP contributions parquet.")
    args = ap.parse_args()
    ...
    df, contribs = score(target, want_contribs=not args.skip_contribs)
    ...
    if contribs is not None:
        c_path        = args.out / f'contributions_{target.date()}.parquet'
        c_latest_path = args.out / 'contributions_latest.parquet'
        contribs.to_parquet(c_path, index=False)
        shutil.copyfile(c_path, c_latest_path)
    ...
    if not args.quiet:
        ...
        if contribs is not None:
            print(f'wrote data/predictions/contributions_{target.date()}.parquet  '
                  f'({len(contribs):,} rows × {contribs.shape[1]} cols, TreeSHAP for y_24h)')
            print(f'wrote data/predictions/contributions_latest.parquet')
